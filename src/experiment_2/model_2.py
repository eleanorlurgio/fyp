...
def get_bias(): # evaluate bias of model
    ...
    output_dir = Path("results/experiment_2/model_2") # set output directory
...
def train_model():
    ...
    for epoch in range(n_epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), "experiment_2_model_2.pt")
        print(f"epoch: {epoch}")
    ...
    output_dir = Path("results/experiment_2/model_2")
...
def load_model():
    model.load_state_dict(torch.load("experiment_2_model_2.pt")) # load previously trained model from memory
