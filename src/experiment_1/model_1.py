...
tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased") # load bert-base-uncased autotokenizer

max_input_length = tokenizer.max_model_input_sizes['bert-base-uncased'] # max input size of bert-base-uncased
...
transformer = transformers.AutoModel.from_pretrained("bert-base-uncased") # import pretrained bert-base-uncased model

output_dim = len(train_data["label"].unique())
freeze = False

model = Transformer(transformer, output_dim, freeze) # create model using pretrained bert-base-uncased model
...
def get_bias(): # evaluate bias of model
    ...
    output_dir = Path("results/experiment_1/model_1") # set output directory
...
def train_model():
    ...
    for epoch in range(n_epochs):
        ...
        if valid_loss < best_valid_loss:
            ...
            torch.save(model.state_dict(), "experiment_1_model_1.pt")
        print(f"epoch: {epoch}")
    ...
    output_dir = Path("results/experiment_1/model_1")
...
def load_model():
    model.load_state_dict(torch.load("experiment_1_model_1.pt")) # load previously trained model from memory
